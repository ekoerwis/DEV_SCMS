...
class logSheetCPOStorageTankModel extends \App\Models\BaseModel
{
    ...
    public function reportSqlString($tdate='' ,$dt_div=''){
        ...
        FROM (
        SELECT SUMSTGID,REGEXP_SUBSTR(SUMSTGID,'[^|]+',1,2) SUMSTGID_UEP
        , TO_CHAR(FS_CONV_UTCUEP2WIB(REGEXP_SUBSTR(SUMSTGID,'[^|]+',1,2)),'DD/MON/YYYY HH24:MI:SS')  SUMSTGID_UEP_TIME
        , TO_CHAR(FS_CONV_UTCUEP2WIB(REGEXP_SUBSTR(SUMSTGID,'[^|]+',1,2)),'DDMONYYHH24')  SUMSTGID_UEP_TIME_H
        , MIN(REGEXP_SUBSTR(SUMSTGID,'[^|]+',1,2)) OVER (PARTITION BY TO_CHAR(FS_CONV_UTCUEP2WIB(REGEXP_SUBSTR(SUMSTGID,'[^|]+',1,2)),'DDMONYYHH24')) SUMSTGID_UEP_TIME_MIN
